...
async function runNotify(env, pkg, latestVersion) {
    ...
    const padding = 3;
    const longestLineLength = Math.max(...lines.map(line => cli_framework_output_1.stringWidth(line)));
    const horizontalRule = `  ${'─'.repeat(longestLineLength + padding * 2)}`;
    const output = (`\n${horizontalRule}\n\n` +
        `${lines.map(line => `  ${' '.repeat((longestLineLength - cli_framework_output_1.stringWidth(line)) / 2 + padding)}${line}`).join('\n')}\n\n` +
        `${horizontalRule}\n\n`);
